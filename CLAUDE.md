...
**Autorização permanente para publicar patch sozinho (23/08/2026).** Pra toda mudança que seja só Dart (patchável, sem alterar Gradle/AndroidManifest/plugin nativo) e que já tenha passado em `flutter analyze` + `flutter test`: disparar o `shorebird-patch.yml` sozinho via `gh workflow run shorebird-patch.yml -f release_version=<versão do release ativo>` (ver versão atual no `pubspec.yaml`), sem pedir confirmação antes — só avisar quando o patch terminar (sucesso ou falha). Isso **não** vale pra release completo (mudança nativa) — aí sempre confirmar antes de disparar `shorebird-release.yml`, porque exige reinstalação manual do fundador no aparelho.
