...
**R8/minificação religada (23/08/2026).** `android/app/build.gradle.kts`: `isMinifyEnabled`/`isShrinkResources` voltaram a `true`, com `proguard-rules.pro` (vazio de propósito — só recebe regra específica se algo quebrar em runtime, nunca `-keep` genérico "por garantia"). Motivo original de ter desligado (build local lento nesta máquina) não existe mais desde que os releases passaram a rodar na nuvem. **É mudança nativa** (Gradle) — só entra em vigor num release completo, nunca num patch (um patch não pode alterar como o APK/AAB já instalado foi empacotado). Versão bumped pra `0.1.4+4` (primeira a usar o esquema `x.y.build+build`). Precisa de teste real num aparelho depois de publicado — se algo quebrar, primeiro passo é voltar `isMinifyEnabled = false` pra confirmar a causa antes de escrever qualquer regra nova.
