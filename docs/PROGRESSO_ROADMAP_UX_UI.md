...
| 8 | Modelos de orçamento | ✅ Feito (28/08/2026) | Schema v6→v7 (`budget_templates`/`budget_template_items`, ver `budget_templates_table.dart`). Modelo nasce a partir de um orçamento existente ("Salvar como modelo" no menu ⋮, `budget_form_screen.dart`) — snapshot de itens/desconto/observações/descrição da obra, nunca editado depois. `BudgetTemplatesScreen` ("Meus Modelos", acessível pelo ícone na aba Orçamentos) lista/exclui; em modo seleção (`selectionMode: true`) devolve o modelo escolhido via `Navigator.pop`. Wizard Etapa 1 ganhou "Usar modelo salvo" no estado vazio, que aplica o modelo ao rascunho em andamento. **Sem paywall por enquanto** — módulo Purchases só existe a partir da Fase 4, travar agora seria placebo. Testado em `test/repositories/budget_templates_repository_test.dart` (3 casos: criar do orçamento, aplicar ao orçamento novo, exclusão lógica). |
...
- **"Medição" virou "cômodo" na UI + "Casa do Cliente" nova tela (01/09/2026)**: medição em obra civil já tem outro significado (boletim financeiro de % executada), confundia o público. Nome de tabela/classe no banco continua `Measurement` (é dado, não é UI). Nova tela `HouseScreen` (ícone de casa na ficha do cliente) lista os cômodos medidos com resumo agregado (soma de m² piso/teto/parede via `MeasurementsRepository.watchHouseTotals()`), separando medição da timeline de orçamentos. Tela de cômodo (`measurement_form_screen.dart`) ganhou painel "Grandezas Derivadas" recalculado ao vivo a cada tecla (antes só salvava cego). Achado de depuração: `AppStatusChip` sem `Flexible` numa `Row` com `Spacer()` estourava em 320dp quando o texto do selo ficava sempre visível (antes só aparecia condicionalmente) — se outra tela usar esse padrão, envolver o chip em `Flexible`.
